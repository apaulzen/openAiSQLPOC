  ...
  RunnablePassthrough.assign({
    query: async (input: { question: string; schemaInfo: string }) => {
      ...
      return await writeQuery.invoke({
        question: `${formattedQuestion}. Limit fetched data to maximum 10 rows. NEVER EVER QUERY ANY IMAGE COLUMN EEVEN IF YOU NEED TO.`,
...
export const contextAwareQnA = async (question: string) => {
  const schemaInfo = `
    ...
    ${db.getTableInfo(db.allTables.map((table) => table.tableName))}
    `;
